...
class Main {
    ...
    static private void findWordsUtil(boolean visited[][],
        ...
        Stack<Nodehere> myStack = new Stack<>(); //create a new node in the stack
        //System.out.println("hi");
        myStack.push(new Nodehere(visited, i, j, str)); //push the gridNode into the stack


        while (!myStack.isEmpty()) {
            Nodehere top = myStack.pop();
            //System.out.println("hello");
            i = top.getRow(); //store the row value into current i
            j = top.getCol(); //store the col value into current j
            str = top.getStr(); //store the string into current str
            visited = top.isVisited(); //store the boolean isVisited into current visited
                ...
                if (row >= 0) {
                    for (int col = j - 1; col <= j + 1 && col < gridSize; col++)
                        if (col >= 0 && !visited[row][col]) {
                            ...
                            myStack.push(new Nodehere(visited, row, col, str)); //push a new node to the stack
                        }
                }
            }
